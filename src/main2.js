...
let game = new Phaser.Game(600, 400, Phaser.AUTO, 'TutContainer', {preload: preload, create: create, update: update});
let upKey;
let downKey;
let leftKey;
let rightKey;
...
let dX = 0;
let dY = 0;
let tileWidth = 34;// the width of a tile
let borderOffset = new Phaser.Point(250, 50);//to centralise the isometric level display
let floorGraphicWidth = 64;
let floorGraphicHeight = 38;
let heroGraphicWidth = 41;
let heroGraphicHeight = 62;
let heroHeight = (floorGraphicHeight / 2) + (heroGraphicHeight - floorGraphicHeight);//adjustments to make the legs hit the middle of the tile for initial load
...
function create() {
    ...
    floorSprite = game.make.sprite(0, 0, 'tileset', 'sand');
    wallSprite = game.make.sprite(0, 0, 'tileset', 'bush1');
    waterSprite = game.make.sprite(0, 0, 'tileset', 'water');
    waterSprite.alpha = 0.5;
    game.add.tween(waterSprite).to({alpha: 1}, 1500, Phaser.Easing.Linear.None, true, 0, 1000, true);
...
function renderScene() {
    ...
    normText.text = 'Player is on x,y: ' + heroMapTile.x + ',' + heroMapTile.y;
}

function drawHeroIso() {
    let heroCornerPt = new Phaser.Point(heroMapPos.x - heroMapSprite.width / 2, heroMapPos.y - heroMapSprite.height / 2);
    let isoPt = cartesianToIsometric(heroCornerPt);//find new isometric position for hero from 2D map position
    gameScene.renderXY(sorcererShadow, isoPt.x + borderOffset.x + shadowOffset.x, isoPt.y + borderOffset.y + shadowOffset.y - 10, false);//draw shadow to render texture
...
function drawTileIso(tileType, i, j) {//place isometric level tiles
    ...
    if (tileType === 1) {
        gameScene.renderXY(wallSprite, isoPt.x + borderOffset.x, isoPt.y + borderOffset.y - 14, false);
    } else if (tileType === 0) {
        ...
        gameScene.renderXY(waterSprite, isoPt.x + borderOffset.x, isoPt.y + borderOffset.y + offset, false);
    } else {
        gameScene.renderXY(floorSprite, isoPt.x + borderOffset.x, isoPt.y + borderOffset.y, false);
    }
}
